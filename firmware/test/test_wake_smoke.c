...
int main(void)
{
    ...
    /* The firmware refuses to run the generated path when the chip's own
       ...
       the reserve has to exist. It is not carved out of the arena: every
       generated model in an image shares one scratch region, because esp-nn
       reaches it through file-static globals. The reserve's actual size can
       only be checked against esp-nn on the device -- the ANSI kernels this
       test links against ask for no scratch at all, so the generated query
       answers 0 here and only its linkage is exercised. */
    assert(WAKE_INFER_SCRATCH_BYTES > 0);
    assert(wake_infer_scratch_query() <= WAKE_INFER_SCRATCH_BYTES);
    wake_infer_init();
    wake_infer_reset();
