...
.. req:: Generated inference is bit-exact with the interpreter
   ...
   With ``CONFIG_KWS_INFER_GENERATED=y`` the wake model runs as C generated by
   ``kws-codegen`` (``firmware/main/gen/wake_infer.c``) calling esp-nn's
   ESP32-S3 kernels directly, not through ``tflite::MicroInterpreter``; the
   command model is generated the same way but is not wired into the firmware
   yet. Every output byte is identical to the interpreter's for the same
