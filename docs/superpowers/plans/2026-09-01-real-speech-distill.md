...
`docs/DATASHEET.md` `## Collection`: one paragraph describing the v3 path (tarball mining by keyword folder with VALID filter, self-recordings with `rec:` speaker ids, TTS as backstop only — `say` + Piper voices discovered from the local cache, multi-speaker Piper voices expanded per speaker, TTS speaker id `tts:<engine>:<voice>` so the split holds out whole voices, one pitch/tempo-perturbed copy per TTS clip), with the note that the v3 real/TTS-per-word table and voice count are written here once the v3 build has run.

`docs/paper-notes.md`: one more entry `### TTS breadth + perturbation (feat/real-speech-distill)` — the `say`-only finding (9 voices × 9 rates, 6 622 clips, rate-in-speaker-id split leak), the fix (Tasks 10–11), and "effect measured with v3".

`docs/paper-notes.md`: under `## Open questions` (create the heading at the end of the file if absent), add `- Grouped speaker k-fold evaluation (spec §9): single split tests few independent real voices, effective n ≈ (speaker, word) pairs; `kws-benchmark --folds 5` over real speakers only, TTS always train-side, mean ± std + per-speaker table. Build after v3 once ≥ 5 speaker groups cover every command.`
Same list, second line: `- Probabilistic slot decoding (spec §10): detector thresholds before the grammar can weigh in; n-best lattice parse over the existing posteriors (≤ 8 sequences per phrase, score = ∏ probs, accept on tau/delta, temperature-calibrated), E11 offline re-decode of the catalog eval with false-accept rate on negatives as the gate; catalog DP decoding only if > 5 points remain.`
...
### Task 10: Piper breadth — cache-discovered voices, multi-speaker expansion, native tempo, honest TTS speaker ids

Spec: §3.5. Executed after Task 8, before Task 9.

**Files:**
- Modify: `kws_de/tts.py` (`ENGINE_VOICES`, `voice_combos`, `_piper_voice_path`, `_piper_load_voice`, `_piper_say`, `synthesize`)
- Modify: `kws_de/data.py` (`_tts_combo_plan`, `_tts_fill_word`)
- Test: `tests/test_tts.py`

**Interfaces:**
- Consumes: `tts.ENGINE_VOICES`, `tts.RATES`, `config.DATA_DIR`.
- Produces: `tts.piper_voices(root: Path) -> list[str]`, `tts.engine_voices(engine: str) -> list[str]`; voice ids of the form `de_DE-mls-medium#17` for multi-speaker Piper voices; TTS speaker ids `tts:{engine}:{voice}` (no rate) — consumed by `split_three_way` / `_origin_flags` unchanged.

- [ ] **Step 1: Write the failing tests**

Append to `tests/test_tts.py` (extend the import line to `from kws_de import config` + `from kws_de.tts import ENGINE_VOICES, _piper_voice_path, engine_voices, piper_voices, voice_combos`):
...
Run: `cd ~/src/kws-de && uv run pytest tests/test_tts.py -q`
Expected: FAIL — `ImportError: cannot import name 'engine_voices'` (the whole file errors on import).

- [ ] **Step 3: Implement in `kws_de/tts.py`**

Replace the module-level `ENGINE_VOICES["piper"]` comment block so it reads as the fallback it now is (keep the same four ids):
...
(`import json` and `from pathlib import Path` at the top if not already there.)

In `voice_combos`, change the two lines that build `per_engine`:
...
(rates-outer: the first `len(voices)` combos cover every voice once.)

In `_piper_voice_path`, first line of the body:

```python
    voice = voice.split("#", 1)[0]  # "<id>#<speaker_id>" -> the shared model file
```

In `_piper_load_voice`, key the memo cache on the base id as well: `voice = voice.split("#", 1)[0]` as the first statement, before the cache lookup.

In `_piper_say`, take a `rate` argument and pass speaker + tempo natively:
...
def _piper_say(word, voice, rate, out_wav):  # pragma: no cover - loads model, runs onnxruntime
    ...
    cfg = SynthesisConfig(
        speaker_id=int(sid) if sid else None, length_scale=PIPER_BASE_RATE / rate
    )
    try:
...
(rest of the body unchanged.) In `synthesize`, the piper branch becomes `return _piper_say(word, voice, rate, out_wav)`; update the module docstring line for piper to "Piper neural voices (cache-discovered, multi-speaker aware)". Update `RATES`' trailing comment: `# wpm; say maps directly, piper via length_scale = 160/rate`.

- [ ] **Step 4: Implement in `kws_de/data.py`**

`_tts_combo_plan`: `pool = min(len(tts.engine_voices(e)) * len(tts.RATES) for e in engines)`.

`_tts_fill_word`: speaker id without the rate, and the docstring to match:

```python
        return None if audio is None else (audio, f"tts:{engine}:{voice}")
```

Docstring: `[(np.ndarray, speaker_id)] with speaker_id="tts:{engine}:{voice}" — rate is augmentation, not identity, so the speaker-disjoint split holds out whole voices.`

- [ ] **Step 5: Run the tests**

Run: `cd ~/src/kws-de && uv run pytest tests/test_tts.py tests/test_data.py -q`
Expected: all pass (existing `_tts_combo_plan` and `voice_combos` tests still hold — they count engines and distinctness, not order).

- [ ] **Step 6: Download the four extra Piper voices and smoke-test one multi-speaker id**

```bash
cd ~/src/kws-de && for v in kerstin/low mls/medium pavoque/low thorsten_emotional/medium; do
  name=${v%/*}; q=${v#*/}; id=de_DE-$name-$q; d=data/piper-voices/$name/$q; mkdir -p $d
  for ext in onnx onnx.json; do
    [ -f $d/$id.$ext ] || curl -fL -o $d/$id.$ext "https://huggingface.co/rhasspy/piper-voices/resolve/main/de/de_DE/$name/$q/$id.$ext"
  done
done
uv run python -c "
from pathlib import Path
from kws_de import config, tts
vs = tts.piper_voices(config.DATA_DIR / 'piper-voices'); print(len(vs), vs[:2], vs[-1])
a = tts.synthesize('licht', 'piper', 'de_DE-mls-medium#17', 200, Path('/tmp/kws_smoke.wav'))
print(None if a is None else (a.shape, a.dtype))"
```

Expected: `~250 ['de_DE-eva_k-x_low', 'de_DE-karlsson-low'] de_DE-thorsten_emotional-medium#7` and a `(N,) float32` shape (not `None`). Downloads land in gitignored `data/`; nothing to commit from this step.

- [ ] **Step 7: Lint and commit**

```bash
cd ~/src/kws-de && uv run ruff check . && uv run ruff format --check . && uv run pytest -q && git add kws_de/tts.py kws_de/data.py tests/test_tts.py && git commit -m "feat(tts): cache-discovered Piper voices, per-speaker ids, native tempo; TTS speaker id drops rate"
```

---

### Task 11: Pitch/tempo perturbation for TTS clips

Spec: §3.5. Executed after Task 10, before Task 9.

**Files:**
- Modify: `kws_de/augment.py` (add `perturb`)
- Modify: `kws_de/data.py` (`build_dataset`, `_origin_flags`)
- Modify: `kws_de/dataset.py` (`assemble`)
- Test: `tests/test_augment.py`, `tests/test_data.py`, `tests/test_dataset.py`

**Interfaces:**
- Consumes: `build_dataset`, `_origin_flags`, `assemble` as they are on the branch.
- Produces: `augment.perturb(sig, n_steps: float, rate: float, sr: int = 16000) -> np.ndarray`; `build_dataset(..., synthetic: dict[str, list[bool]] | None = None)`; `_origin_flags(clips_ws, snrs, words=None, perturb_tts=False)`.

- [ ] **Step 1: Write the failing tests**

Append to `tests/test_augment.py` (import `perturb` from `kws_de.augment`):
...
Run: `cd ~/src/kws-de && uv run pytest tests/test_augment.py tests/test_data.py tests/test_dataset.py -q`
Expected: `test_augment.py` errors on import (`perturb`); the two `test_data.py` tests fail with `TypeError: ... unexpected keyword argument`; the `test_dataset.py` test fails on the `is_tts.sum()` count (4, not 8).

- [ ] **Step 3: `augment.perturb`**

Append to `kws_de/augment.py`:
...
- [ ] **Step 4: `build_dataset` + `_origin_flags` in `kws_de/data.py`**

Add `from kws_de.augment import mix_at_snr, perturb` (replace the existing import line). Give `build_dataset` a trailing `synthetic=None` parameter and add to its docstring:
...
Replace `add_word_clip` and the two loops that call it:
...
`_origin_flags`: signature `(clips_ws: dict, snrs, words=None, perturb_tts: bool = False)`; docstring gains "with `perturb_tts`, TTS clips count twice (build_dataset's perturbed copy)"; the two per-clip loops become:
...
- [ ] **Step 5: `assemble` in `kws_de/dataset.py`**
...
```python
    ...
    X, y = build_dataset(
        clips, noises, rng, labels=labels, commands=commands, synthetic=synthetic
    )
    is_tts = _origin_flags(clips_ws, snrs=(20, 10, 0), words=commands, perturb_tts=True)
```

Docstring: add "TTS clips get one perturbed copy (build_dataset `synthetic`), mirrored in the flags."

- [ ] **Step 6: Run the tests**

Run: `cd ~/src/kws-de && uv run pytest -q`
Expected: all pass, including the untouched v1 tests (no `synthetic` → identical rows and rng stream).

- [ ] **Step 7: Lint and commit**

```bash
cd ~/src/kws-de && uv run ruff check . && uv run ruff format --check . && git add kws_de/augment.py kws_de/data.py kws_de/dataset.py tests/test_augment.py tests/test_data.py tests/test_dataset.py && git commit -m "feat(data): pitch/tempo-perturbed copy of every TTS clip at build time"
```

---

### Task 12: Training throughput on the Mac — batch size, TFLite threads, Metal check

Executed after Task 11, before Task 9, so Task 9's long runs use the faster settings and all E9/E10 numbers share one configuration. Findings so far: TF 2.21 sees `CPU:0` only (`tensorflow-metal` never synced; pyproject has the `metal` extra), `batch_size=32` hard-coded in `train.py` and `distill.py`, `tf.lite.Interpreter` created single-threaded on a 10-core M4.

**Files:**
- Modify: `kws_de/config.py` (add `BATCH_SIZE`)
- Modify: `kws_de/train.py:45` (`batch_size=32` → `config.BATCH_SIZE`)
- Modify: `kws_de/distill.py` (`batch_size=32` in `student.fit` → `config.BATCH_SIZE`)
- Modify: `kws_de/budgets.py:25`, `kws_de/eval.py:125`, `kws_de/eval.py:272` (interpreter threads)
- Modify: `docs/paper-notes.md` (timing entry)
- Modify: `docs/paper.md` (one sentence: batch size from E9 on)

**Interfaces:**
- Consumes: `train.train(...)`, `distill.distill(...)` (Task 8), `tf.lite.Interpreter` call sites.
- Produces: `config.BATCH_SIZE: int = 128`. No signature changes. `kws_de/wake.py:28` (on-device-style runtime path) is left single-threaded on purpose.

- [ ] **Step 1: Batch size constant**

`kws_de/config.py`, next to the other training constants:
...
`kws_de/train.py`: `batch_size=config.BATCH_SIZE,` in `model.fit`. `kws_de/distill.py`: same in `student.fit`. (`config` is already imported in both.)

- [ ] **Step 2: Run the suite; pin toy tests that need small batches**

Run: `cd ~/src/kws-de && uv run pytest -q`
Expected: pass. If a toy-data test (a few dozen samples) now fails its "loss decreases / accuracy > 0.9" assertion because one epoch is one step, pass `batch_size=32` through explicitly for that test only — add a `batch_size: int | None = None` kwarg to the function it calls, defaulting to `config.BATCH_SIZE` — and leave production callers at the default. Do not raise epochs to compensate.

- [ ] **Step 3: Interpreter threads**

At `kws_de/budgets.py:25`, `kws_de/eval.py:125`, `kws_de/eval.py:272`, add `num_threads=os.cpu_count()` to the `tf.lite.Interpreter(...)` call (`import os` where missing). `wake.py` unchanged. Results are bit-identical; only wall-clock changes.

Run: `cd ~/src/kws-de && uv run pytest -q tests/test_budgets.py tests/test_eval.py`
Expected: pass.

- [ ] **Step 4: Time CPU@32 vs CPU@128 vs Metal@128**

Features are the frozen v2 `data/features_{train,val,test}.npz`. Measure a 2-epoch `kws-train` three ways and note the seconds per epoch (`time`, wall-clock, minus the constant load time — take epoch 2's time from a `verbose=1` run or just compare totals):

```bash
cd ~/src/kws-de
git stash -q -- kws_de/config.py kws_de/train.py kws_de/distill.py   # batch 32 baseline
time uv run kws-train --epochs 2
git stash pop -q                                                        # batch 128
time uv run kws-train --epochs 2
uv sync --extra metal
...
Decision rule: keep `--extra metal` only if the GPU device is listed AND the run is ≥ 1.3× faster than CPU@128; otherwise `uv sync` (drops the plugin) and record why (plugin refuses to load against TF 2.21, or no gain at this model size). `uv.lock` does not change either way — extras are already resolved in it. Do not commit `data/` or `models/` output.

- [ ] **Step 5: Record**

`docs/paper-notes.md`, new entry `### Training throughput (feat/real-speech-distill)`: date, machine (Apple M4, 10 cores, 16 GB — hardware only, no paths), the three timings in a 3-row table (`| config | s/epoch |`), the Metal decision with its reason, and the sentence "batch 128 from E9 on; E7 numbers were batch 32 — not re-run".

`docs/paper.md`: in the experimental-setup paragraph, add "Batch size 32 through E8, 128 from E9 on (throughput; §notes)".

`README.md` line 29 comment stays; if Metal was kept, change it to `# --extra metal: Apple-GPU, ~Nx faster here`, N from Step 4.

- [ ] **Step 6: Lint and commit**

```bash
cd ~/src/kws-de && uv run ruff check . && uv run ruff format --check . && npx --yes markdownlint-cli@0.42.0 --config .markdownlint.json docs/paper.md docs/paper-notes.md README.md && git add kws_de/config.py kws_de/train.py kws_de/distill.py kws_de/budgets.py kws_de/eval.py docs/paper.md docs/paper-notes.md README.md && git commit -m "perf: batch 128, multi-threaded TFLite eval; Metal measured"
```

Not in this task: parallelising the Task 11 perturbation in the dataset build (measure the v3 build time first; `ProcessPoolExecutor` over clips only if it exceeds ~15 min).
