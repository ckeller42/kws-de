...
def write_wake_config(tflite: bytes, path) -> dict:
    ...
    probability (prob = q * scale + zero_point, i.e. q/256). Arena estimate
    follows write_model_config(): int8 tensor bytes x1.2 rounded up to 4 KB,
    plus headroom for the streaming ring buffers TFLM keeps as resource
    variables; the device logs arena_used_bytes() so it can be tightened."""
    itp = tf.lite.Interpreter(model_content=tflite)
    itp.allocate_tensors()
    inp, out = itp.get_input_details()[0], itp.get_output_details()[0]
    tensor_bytes = sum(
    ...
    arena = -(-int(tensor_bytes * 1.2) // 4096) * 4096
    arena = max(arena, 32768)  # TFLM's planner needs more than the desktop peak here
