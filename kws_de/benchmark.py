...
from kws_de.eval import _tflite_predict, make_command_predict_fn, run_catalog_eval
...
def render_table(rows: list[dict]) -> str:
    """Pure Markdown comparison table: Architecture/Isolated/Catalog/Params/MACs/INT8/Budget."""
    header = "| Architecture | Isolated | Catalog | Params | MACs | INT8 | Budget |"
    sep = "|---|---|---|---|---|---|---|"
    lines = [header, sep]
    for r in rows:
        budget = "yes" if r["budget_ok"] else "no"
        lines.append(
            f"| {r['name']} | {r['isolated_acc']:.3f} | {r['catalog_acc']:.3f} | "
            f"{r['params']:,} | {r['macs']:,} | {r['int8_bytes']:,} | {budget} |"
        )
    return "\n".join(lines) + "\n"


def evaluate_architecture(name: str, epochs: int = EPOCHS, seed: int = SEED) -> dict:
    # pragma: no cover - heavy I/O (training + TTS + tflite)
    """Build `name`, train on `load_split("train")` (val-selected best epoch),
    INT8-export, and report isolated-word test accuracy, full-intent catalog
    accuracy, and on-device cost (params/MACs/INT8 bytes/budget)."""
    ...
    tflite_bytes = to_int8_tflite(model, X_train[:200])

    y_pred = _tflite_predict(tflite_bytes, X_test)
    isolated_acc = float((y_pred == y_test).mean())

    predict_fn = make_command_predict_fn(tflite_bytes)
    catalog = run_catalog_eval(predict_fn, CATALOG_VOICES, seed=seed)
...
def main() -> None:  # pragma: no cover - I/O wrapper
    ...
    rows = [
        evaluate_architecture(name, epochs=EPOCHS, seed=SEED)
    ...
    intro = (
        ...
        "v2 dataset (`kws_de.dataset.load_split`, `data/manifest.json` seed=0). "
        "**Isolated** = INT8 test-set word accuracy. **Catalog** = full-intent "
